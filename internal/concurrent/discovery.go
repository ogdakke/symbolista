...
func DiscoverFilesV2(rootPath string, matcher *gitignore.Matcher, jobChan chan<- FileJob, asciiOnly bool, collector *ResultCollector, errorCallback func(error)) {
	defer close(jobChan)

	logger.Debug("Starting file discovery (WalkDir)", "root_path", rootPath)
	...
	logger.Debug("File discovery completed (WalkDir)")
}
